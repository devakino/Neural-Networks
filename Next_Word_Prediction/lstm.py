# -------------------------------------------------------------
# 🧠 Email Next-Word Prediction using LSTM with memory
# -------------------------------------------------------------
import torch
...
embed_size = 128
hidden_size = 128
model = EmailLSTM(vocab_size, embed_size, hidden_size)
...
# -------------------------------------------------------------
# 6️⃣ Prediction function with memory
# -------------------------------------------------------------
def predict_next_word(model, text, hidden=None, top_k=3):
    ...
    with torch.no_grad():
        ...
        output, hidden = model(x, hidden)  # pass previous hidden state
...
# -------------------------------------------------------------
# 7️⃣ Interactive next-word prediction loop with memory
# -------------------------------------------------------------
...
while True:
    ...
    if text == "exit":
        print("👋 Goodbye!")
        break
    suggestions, hidden_state = predict_next_word(model, text, hidden_state)
